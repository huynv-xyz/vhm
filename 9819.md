...
| 13 | `NOXH_approver_sales_dept` | `outbox_event.payload.actorId` tại `registration.transitioned` / `action=APPROVE`, `fromState=salesUnderReview`; hoặc `dossier_stage_reviewer` stage SALES | Chưa xác minh occurrence và tên người duyệt; event actor là ứng viên chính xác hơn reviewer hiện tại |
| 14 | `NOXH_approver_procedure_dept` | `outbox_event.payload.actorId` tại `registration.transitioned` / `action=APPROVE`, `fromState=procedurePending`; hoặc `dossier_stage_reviewer` stage PROCEDURE | Chưa xác minh occurrence và tên người duyệt |
| 17 | `NOXH_profile_progress` | Không có cột tiến độ | Chưa xác minh cách tính từ pipeline state |
| 19 | `NOXH_reject_supplement_reason` | `dossier.revision_reason_code`, `latest_reject_reason_code`; `outbox_event.payload.reasonCode/comment` của REJECT/REQUEST_REVISION; `dossier_stage_reviewer.comment` | Chưa xác minh nguồn ưu tiên/chu kỳ; `reasonCode` không có ở mọi event |
| 24 | `NOXH_submitter_name` | `outbox_event.payload.actorId` tại `registration.transitioned` / `action=SUBMIT` (hoặc RESUBMIT); `dossier_status_history.actor_id` chỉ có coarse status | Chưa xác minh submit đầu/resubmit và lookup tên |
| 26 | `NOXH_supplement_request_date` | `outbox_event.payload.occurredAt` tại `registration.transitioned` / `action=REQUEST_REVISION`; `dossier_status_history.at` cho coarse transition | Chưa xác minh lấy lần đầu/lần gần nhất và trường hợp yêu cầu qua stage intake |
...
| 43 | `NOXH_report_sxd_approved_date` | `dossier_stage_reviewer.reviewed_at,decision` stage SXD; `outbox_event.payload.occurredAt` tại APPROVE → approved | Chưa xác minh dùng thời điểm quyết định hay transition; report hiện không kiểm decision |
| 44 | `NOXH_report_arrival_date` | `dossier_note.occurred_at` với `kind=HARDCOPY_SUBMISSION` hoặc `HARDCOPY_RECEIPT` | Chưa xác minh: hai sự kiện khác nghĩa; không lấy `dossier.submitted_at` thay thế |
...
| 55 | `NOXH_checklist_group_name` | `document_template_sets.properties.registrationGroupId`; có thể lookup `master_data_items` ở Definition Center | Chưa xác minh chiều group, liên kết ID và nguồn tên |
| 56 | `NOXH_checklist_target_group` | `document_template_sets.properties.applicantTypeId`; có thể lookup `master_data_items` ở Definition Center | Chưa xác minh chiều target, liên kết ID và nguồn tên |
