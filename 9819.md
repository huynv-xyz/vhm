...
| STT | Metric Datamart | Cột/JSON path đã tìm thấy | Đề xuất để review (chưa xác nhận) |
| --- | --- | --- | --- |
| 4 | `NOXH_agency_name` | `dossier.form_data.projectRegistration.agencyId` | Tra tên team theo `agencyId` qua Profile MW như report hiện tại; xác nhận team này có đúng là “đại lý/bộ phận kinh doanh” ở cả AGENT và MARKET. |
| 5 | `NOXH_proposed_unit_info` | `dossier.form_data.projectRegistration.proposedUnitCode` | Nếu metric chỉ cần mã căn nguyện vọng, dùng field này. Nếu cần tòa/tầng/diện tích, join quỹ căn bằng project + unit code sau khi chốt thuộc tính. |
| 10 | `NOXH_spouse_eligible_group` | Chưa có field đúng trong form schema | Bổ sung field riêng cho nhóm chính sách của vợ/chồng tại nguồn nhập liệu, hoặc để null nếu nghiệp vụ không thu thập. Không dùng `subjectCoApplicant`. |
| 12 | `NOXH_updated_date` | `dossier.updated_at`; report dùng `last_event_at` | Dùng `updated_at` nếu nghĩa là lần cuối bản ghi thay đổi; dùng `last_event_at` nếu nghĩa là lần cuối có business event. Chọn một theo định nghĩa BA. |
| 13 | `NOXH_approver_sales_dept` | `outbox_event.payload.actorId` của APPROVE từ SALES; reviewer row | Lấy actor của **lần APPROVE SALES gần nhất**, lookup tên; không dùng tên reviewer được phân công nếu chưa approve. |
| 14 | `NOXH_approver_procedure_dept` | `outbox_event.payload.actorId` của APPROVE từ PROCEDURE; reviewer row | Lấy actor của **lần APPROVE PROCEDURE gần nhất**, lookup tên; đối chiếu với reviewer row. |
| 17 | `NOXH_profile_progress` | `dossier.current_stage_code`, pipeline definition | Xuất stage/state hiện tại làm tiến độ dạng nhãn; nếu cần tỷ lệ %, BA phải cung cấp bảng quy đổi và rule khi hồ sơ quay lại bổ sung. |
| 19 | `NOXH_reject_supplement_reason` | Reason code trên dossier; reasonCode/comment trong transition outbox | Lấy lý do của **sự kiện REJECT hoặc REQUEST_REVISION gần nhất**; ưu tiên reason code + nhãn, giữ comment ở field riêng. Chốt xử lý REVOKE_UNIT. |
| 24 | `NOXH_submitter_name` | `outbox_event.payload.actorId` của SUBMIT/RESUBMIT | Lấy actor của **SUBMIT lần đầu** rồi lookup tên; nếu muốn người nộp lần gần nhất thì đổi sang SUBMIT/RESUBMIT cuối. |
| 26 | `NOXH_supplement_request_date` | `outbox_event.payload.occurredAt` của REQUEST_REVISION | Lấy REQUEST_REVISION gần nhất đưa hồ sơ sang `agentUpdateAtSales`; nếu bao gồm intake nội bộ thì mở rộng rule. |
| 37 | `NOXH_report_proposed_unit_info` | `dossier.form_data.projectRegistration.proposedUnitCode` | Dùng đúng rule đã chốt ở metric 5 để hai cột không lệch. |
| 39 | `NOXH_report_agency_name` | `dossier.form_data.projectRegistration.agencyId` | Dùng đúng lookup và snapshot policy đã chốt ở metric 4. |
| 42 | `NOXH_report_updated_date` | `dossier.updated_at` / `last_event_at` | Dùng cùng định nghĩa metric 12; không duy trì hai cách tính khác nhau chỉ vì report cũ dùng `last_event_at`. |
| 43 | `NOXH_report_sxd_approved_date` | SXD reviewer decision/time; APPROVE → approved event time | Lấy thời điểm transition APPROVE → `approved` gần nhất; kiểm chéo `decision=APPROVED` của SXD. |
| 44 | `NOXH_report_arrival_date` | `dossier_note.occurred_at` của HARDCOPY_SUBMISSION/RECEIPT | Nếu “đến nộp” nghĩa là phòng ban đã nhận bản cứng, lấy **HARDCOPY_RECEIPT đầu tiên còn hiệu lực**; nếu chỉ là Đại lý báo nộp, dùng HARDCOPY_SUBMISSION. |
| 45 | `NOXH_report_due_date_1` | `entered_stage_at` + pipeline SLA/lịch nghỉ | Chỉ dùng `RevisionSlaView.firstDueAt` nếu BA xác nhận “đến hạn xử lý” chính là hạn bổ sung lần 1; nếu không, cần rule deadline xử lý riêng. |
| 46 | `NOXH_report_overdue_days_1` | `firstDueAt` + thời điểm tính | Cùng rule 45; tính theo `as_of_at` và dừng khi chu kỳ xử lý kết thúc nếu BA xác nhận. |
| 47 | `NOXH_report_due_date_2` | `entered_stage_at` + pipeline SLA/lịch nghỉ | Chỉ dùng `RevisionSlaView.secondDueAt` nếu cùng nghĩa hạn bổ sung lần 2; nếu không, cần rule riêng. |
| 48 | `NOXH_report_overdue_days_2` | `secondDueAt` + thời điểm tính | Cùng rule 47; tính theo `as_of_at` và trạng thái chu kỳ. |
| 53 | `NOXH_doc_template_file` | `document_templates.s3_file_paths` | Xuất **mảng S3 path nội bộ** dạng JSON nếu Datamart hỗ trợ; không biến thành một file/link tạm khi nguồn có nhiều file. |
| 55 | `NOXH_checklist_group_name` | `document_template_sets.properties.registrationGroupId`; Definition Center `master_data_items` | Thử resolve `registrationGroupId` sang `master_data_items.name` sau khi đối chiếu đúng master-data code và tenant/org. |
| 56 | `NOXH_checklist_target_group` | `document_template_sets.properties.applicantTypeId`; Definition Center `master_data_items` | Thử resolve `applicantTypeId` sang `master_data_items.name` sau khi đối chiếu đúng master-data code và tenant/org. |
| 58 | `NOXH_checklist_quantity` | `document_template_sets.document_templates` | Đếm toàn bộ phần tử JSON của checklist; nếu nghiệp vụ muốn số giấy tờ **bắt buộc**, chỉ đếm `isRequired=true`. |
