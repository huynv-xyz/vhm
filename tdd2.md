...
## Hướng dẫn đọc, triển khai và phê duyệt

Không yêu cầu mọi bên đọc tuần tự toàn bộ TDD. Mỗi nhóm sử dụng phạm vi dưới đây;
chỉ được xác nhận hoàn thành khi có đủ bằng chứng tương ứng.

| **Đối tượng** | **Phần bắt buộc đọc** | **Nội dung phải triển khai/phê duyệt** | **Bằng chứng hoàn thành** |
| --- | --- | --- | --- |
| **Product/Domain Owner** | 1.2; 2.1.6–2.1.8; 3.2–3.4; 4.6–4.7; A.1; B | Journey, use case, consent purpose, business decision, field scope, retry/manual-review policy và domain event | Baseline Product/Domain được ký duyệt; UAT outcome; consumer registration hoàn chỉnh |
| **Architect/System Owner** | 1.2.3–1.2.6; 2; 4; 5.1; 6; 8; B | System boundary, state machine, official-result path, API/event contract, idempotency, resilience, deployment và ADR | Architecture approval; ADR accepted; NFR/RTO/RPO được chốt |
| **Backend/Platform Dev** | 2.2–2.6; 3; 4; 5.2.1; 5.2.6–5.2.12; 5.3 | Session lifecycle, Provider Adapter, callback inbox, normalization, reconciliation, pipeline, manual review và outbox | Unit/integration/contract test; migration; API/event schema; dashboard kỹ thuật |
| **Mobile Dev** | 2.1.7; 3.4; 4.2.1–4.2.3; 4.2.8; 5.2.3; 5.2.5; 5.2.9; 6.6.2; 7.1.3 | Native SDK lifecycle, permission, result page `OFF`, màn hình processing, `submitted`, app resume, handoff và secure manual evidence | Real-device matrix; E2E Mobile; lifecycle/fail-fast test; security evidence |
| **Web Dev** | 2.1.7; 3.4; 4.2.1–4.2.5; 5.2.2; 5.2.4–5.2.5; 5.2.9; 6.6.2; 7.1.3 | Web SDK lifecycle, browser/camera compatibility, result page `OFF`, màn hình processing, refresh/multi-tab, CSP và handoff | Browser matrix; E2E Web; CSP report; refresh/multi-tab/handoff test |
| **SDK Technical Team** | 2.1.7; 4.3–4.5; 6.6.2; A.2–A.3 | SDK profile, completion/close event, callback authentication/schema, Get Result reconciliation, quota, compatibility và retention | Contract/fixture được xác nhận; staging test; profile/config export; support SLA |
| **QA** | 3.2–3.4; 4.6; 5.2–5.3; 6.4.2; 7.4; C | Happy path, callback race/duplicate/lost, retry, fail-fast, manual evidence, Mobile/Web matrix, security và recovery | Test plan có traceability; SIT/UAT report; defect closure; go-live test evidence |
| **ANBM/Security** | 2.1.5; 6.1; 7.1; 7.3–7.4; A.4; C.2 | Trust boundary, credential, callback auth/replay, device/Web security, encryption, evidence access và threat controls | Security review; pentest; exception record; C.2 được ký duyệt |
| **Data Privacy/Legal** | 1.4; 2.1.8; 7.2; A.4; C.3 | Consent/legal basis, data inventory, field scope, residency, retention, deletion và manual-evidence purpose | DPIA/DPA; retention approval; data-flow approval; C.3 được ký duyệt |
| **DevOps/Operations** | 5.3; 6; 8; A.5; C.4 | CI/CD, secret/config, scaling, monitoring, reconciliation workers, backup/recovery, incident và rollback | Deployment/runbook; load/recovery test; dashboard/alert; C.4 được ký duyệt |
